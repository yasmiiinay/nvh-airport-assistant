...
Writes to outputs/checkpoint_03_3/speech/. With no manifest rows the script
writes STATUS.md saying BLOCKED BY DATA and stops. Otherwise, per split:
...
from src.speech import SpeechResult, check_audio, load_audio, process_transcript, transcribe

OUT_DIR = SETTINGS.outputs_dir / "checkpoint_03_3" / "speech"
...
def main() -> int:
    ...
    if not rows:
        return blocked(f"audio_manifest.csv has no rows for split '{args.split}'. "
    ...
    summary = {
        "split": args.split, "n_clips": len(rows), "n_gate_rejected": len(rows) - len(scored),
